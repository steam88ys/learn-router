...
/*
렌더링하는 과정에서 리다이렉트를 하고 싶은 경우는
Navigate컴포넌트를 그려준다 => 사실은 렌더링이 되는 것은 아니고
Navigate컴포넌트의 to속성으로 전달한 주소로 리다이렉트 작업이 수행됨

렌더링 모두 끝난 후 이벤트 핸들러(ex: 클릭 이벤트)에 의해서
리다이렉트를 하고 싶은 경우는 useNavigate 훅을 통해 전달받은 n
avigate함수를 호출하여 리다이렉트 작업을 수행한다.
*/
